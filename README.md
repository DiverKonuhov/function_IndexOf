...
// Первая строка: 'O','n','e',' ','X' (X на позиции 0,4 — это то, что мы игнорим)
matrix[0, 0] = 'O'; matrix[0, 1] = 'n'; matrix[0, 2] = 'e'; matrix[0, 3] = ' '; matrix[0, 4] = 'X'; 
// Вторая строка
matrix[1, 0] = 'm'; matrix[1, 1] = 'o'; matrix[1, 2] = 'r'; matrix[1, 3] = 'e'; matrix[1, 4] = ' ';
// Третья строка
matrix[2, 0] = 't'; matrix[2, 1] = 'i'; matrix[2, 2] = 'm'; matrix[2, 3] = 'e'; matrix[2, 4] = ' ';

for (int i = 0, j = 0; j < 3; )
{
    // ПРОВЕРКА: если это НЕ строка 0 столбец 4 — печатаем
    if (!(j == 0 && i == 4)) 
    {
        Console.Write(matrix[j, i]);
    }
